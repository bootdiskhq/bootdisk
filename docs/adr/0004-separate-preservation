...
what Bootdisk possesses or has observed;
where an observed object came from;
what we believe the object represents.
Decision
...
Preservation must not depend on successful catalog identification.
Catalog interpretation must not alter original preservation observations.
Source provenance must not define Artifact identity.
Source-specific metadata formats belong in adapters rather than the core model.
Standalone software must be a first-class use case.
Unknown or incomplete information must be representable without inventing values.
Derived and interpreted information should be distinguishable from observed source data.
Original media must never be modified by ingest.
