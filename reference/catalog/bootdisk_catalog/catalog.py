...
class LoadedRecord:
    """A record plus its source file for diagnostics only.

    `path` is deliberately not exposed as record identity. It is retained solely
    so validation failures can point humans to the file that needs correction.
    """
...
class Catalog:
    ...
    def load(cls, root: str | Path) -> "Catalog":
        """Load every JSON file beneath *root*.

        Directory names and filenames have no semantic meaning. A record is
        addressed exclusively by its internal stable `id`.
        """
    ...
    def record(self, record_id: str) -> dict[str, Any]:
        """Return a record by stable catalog ID."""
    ...
    def _validate_source_ref(
        ...
        # A path is useful evidence, but it is never accepted as source identity.
